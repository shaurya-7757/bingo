...
export async function signUp({ email, password, username }) {
  ...
  // Local/Offline Fallback if Supabase not configured
...
export async function signIn({ email, password }) {
  ...
  if (isSupabaseConfigured) {
    ...
    if (data?.user) {
      ...
      const username = profileData?.username || data.user.user_metadata?.username || cleanEmail.split("@")[0];
      const profile = {
        id: data.user.id,
        email: data.user.email,
        username,
...
export async function getCurrentUser() {
  if (isSupabaseConfigured) {
    ...
    if (data?.session?.user) {
      ...
      const username = profileData?.username || user.user_metadata?.username || user.email?.split("@")[0] || "Player";
      return {
        id: user.id,
        email: user.email,
        username,
